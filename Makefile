...
# - Exposes port 8080 for access at http://localhost:8080
	docker build . -f Dockerfile.dev -t alerts-service && docker run -p 8080:8080 -v ./src:/app -v ./.env:/app/.env alerts-service
...
# - Port 8080 is exposed but primarily used for any test server needs; reports are generated in ./reports.
	docker build . -f Dockerfile.run_test -t alerts-service && docker run -p 8080:8080 -v ./reports/:/app/reports alerts-service
