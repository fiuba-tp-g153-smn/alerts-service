...
def _cuarteron_up_to_date(out_path: str, svg_path: str) -> bool:
    """Whether the cached cuarteron.png is present and not older than its source SVG.
...
async def _build_cuarteron_cache(settings, logger: Logger) -> None:
    ...
    os.makedirs(cache_dir, exist_ok=True)
    out_path = os.path.join(cache_dir, _CUARTERON_CACHE_NAME)
    if _cuarteron_up_to_date(out_path, _CUARTERON_SVG_PATH):
        logger.info(
            "cuarteron.png already exists and is up to date — skipping rebuild."
        )
        return
