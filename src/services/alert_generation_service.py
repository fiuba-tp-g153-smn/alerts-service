...
_WORKER_PATH = os.path.join(os.path.dirname(__file__), "..", "alert_generation_worker.py")
...
class AlertGenerationService:
    ...
    async def generate_alert(self, geometry: dict, fenomeno_codigo: int) -> dict:
       ...
       - fenomeno_codigo: Input phenomenon code
        ...
        departments = await self.geo_service.intersect_departments(geometry, simplified=True)
        ...
        taviso_id = self.mysql_repo.insert_taviso(fenomeno_text, area_html, poligono_str)
    ...
    def _filter_partidos_by_departments(
        ...
        for p in all_partidos:
            ...
            hit = any(dg.contains(pt) for dg in dept_geoms) if dept_geoms else input_geom.contains(pt)
            if hit:
                resultado.append(p)
