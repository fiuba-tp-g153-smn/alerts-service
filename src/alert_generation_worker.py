...
def _dept_geoms_en_bbox(dept_index: list, lon_o, lon_e, lat_s, lat_n) -> list:
    ...
    return [
        g for (bx0, by0, bx1, by1), g in dept_index
...
def _panel_aviso(fig, texto, modo="area"):
    ...
    if modo == "gral":
        ax2.text(
            0.97, 0.80, "ZONA: AREA TOTAL",
            ha="right", va="center",
            fontsize=11, fontweight="bold", fontstyle="italic",
            fontfamily="monospace",
            antialiased=False,
            color="#000000", transform=ax2.transAxes
        )
...
def generar_gif_area(texto, coords, partidos, timestamp, output_dir, todos_partidos,
                     dept_index, prov_geoms):
    ...
    ax.add_feature(cfeature.BORDERS.with_scale("50m"), edgecolor="black", linewidth=1.8, zorder=4)
    ax.add_feature(cfeature.COASTLINE.with_scale("50m"), edgecolor="black", linewidth=0.8, zorder=4)

    # Department boundaries filtered to visible bbox
    dept_vis = _dept_geoms_en_bbox(dept_index, lon_o, lon_e, lat_s, lat_n)
    if dept_vis:
        ax.add_geometries(dept_vis, crs=ccrs.PlateCarree(),
                          edgecolor="black", facecolor="none", linewidth=0.8, zorder=4)
    ...
    ax.add_patch(MplPolygon(xy, closed=True, facecolor="none",
                            edgecolor="#CC0000", linewidth=0, hatch="//",
    ...
    for p in (todos_partidos or []):
        ...
        ax.plot(lo, la, marker, color=color_pt, markersize=msize,
                transform=ccrs.PlateCarree(), zorder=zpt)

        ax.text(lo + 0.04, la + 0.03, p["nom_partido"],
                fontsize=7.5, color=color_txt, fontweight="bold",
                transform=ccrs.PlateCarree(), zorder=ztxt, clip_on=True,
    ...
    fig.savefig(tmp, format="png", bbox_inches=None, pad_inches=0, facecolor="white", dpi=80)
    plt.close(fig)
...
def generar_gif_general(texto, coords, timestamp, output_dir, dept_geoms, prov_geoms):
    ...
    ax_map.add_feature(cfeature.BORDERS.with_scale("50m"), edgecolor="black", linewidth=1.8, zorder=4)
    ax_map.add_feature(cfeature.COASTLINE.with_scale("50m"), edgecolor="black", linewidth=1.4, zorder=4)

    # All department boundaries
    if dept_geoms:
        ax_map.add_geometries(dept_geoms, crs=ccrs.PlateCarree(),
                              edgecolor="black", facecolor="none",
                              linewidth=0.5, zorder=3)

    # All province boundaries (thicker)
    if prov_geoms:
        ax_map.add_geometries(prov_geoms, crs=ccrs.PlateCarree(),
                              edgecolor="black", facecolor="none",
                              linewidth=1.8, zorder=4)

    # Polygon (filled with transparency + border)
    ax_map.add_patch(MplPolygon(xy, closed=True, facecolor="#FF4444", alpha=0.55,
                                edgecolor="#CC0000", linewidth=2.5,
                                transform=ccrs.PlateCarree(), zorder=5))
    ...
    tmp = out.replace(".gif", "_tmp.png")
    fig_final.savefig(tmp, format="png", bbox_inches=None, pad_inches=0, facecolor="white", dpi=80)
    plt.close(fig_final)
    Image.open(tmp).convert("P", palette=Image.ADAPTIVE).save(out, format="GIF")
    os.remove(tmp)
...
def main():
    ...
    try:
        ...
        gif_area = generar_gif_area(
            fenomeno_text, coords, affected_partidos, timestamp, output_dir,
            all_partidos, dept_index, prov_geoms
        )
