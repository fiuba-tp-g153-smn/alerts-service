...
NATIONAL_SEATS = {
    ...
    for (prov, dep), cab in _NATIONAL_SEATS_PAIRS.items()
}
...
def _load_ign_layers(cache_path: str) -> dict:
    ...
    # Deserialise WKB hex → shapely geometries (except 'place_labels' which are dicts)
    layers: dict = {}
    for key, wkb_list in raw.items():
        if key == "place_labels":
            layers[key] = wkb_list  # already dicts {lon, lat, nombre, tipo}
        else:
...
def _load_inset_png(target_px: int = 600) -> np.ndarray | None:
    """Rasterise the corner-inset SVG once per process and return as RGBA array.
...
def generate_area_gif(  # pylint: disable=too-many-locals
    ...
    for department in all_departments or []:
        ...
        etiqueta = _department_label(
            dept_name, department.get("province", ""), lon, lat
        )
